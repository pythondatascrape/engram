...
// defaultConfigYAML is a minimal valid configuration written when no config exists.
// Port values must match defaults() so that Load() succeeds on the written file.
const defaultConfigYAML = `# Engram daemon configuration
# Generated by engram install. Edit to customise.
server:
  port: 4433
proxy:
  port: 4242
...
// If path already exists it is left unchanged.
func EnsureDefault(path string) error {
	...
	if err := os.WriteFile(path, []byte(defaultConfigYAML), 0644); err != nil {
		return fmt.Errorf("config: write default config %q: %w", path, err)
	}
	return nil
}
